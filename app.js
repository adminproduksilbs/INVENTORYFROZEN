...
import{initializeFirestore,getFirestore,collection,getDocs,getDoc,doc,addDoc,setDoc,updateDoc,deleteDoc,serverTimestamp,runTransaction,writeBatch,query,orderBy,limit,persistentLocalCache,persistentMultipleTabManager}from"https://www.gstatic.com/firebasejs/12.1.0/firebase-firestore.js";
...
const QUEUE_KEY="lbs_inventory_pending_v8";
function getQueue(){try{return JSON.parse(localStorage.getItem(QUEUE_KEY)||"[]")}catch{return[]}}
function saveQueue(q){localStorage.setItem(QUEUE_KEY,JSON.stringify(q))}
function queueProduction(data){const q=getQueue();q.push({id:crypto.randomUUID?crypto.randomUUID():String(Date.now()+Math.random()),type:"production",data});saveQueue(q);return q.length}
function updateLocalProduct(id,qty){const p=products.find(x=>x.id===id);if(p)p.stok=Number(p.stok||0)+Number(qty||0);localStorage.setItem("lbs_inventory_products_cache_v8",JSON.stringify(products))}
function cacheProducts(){try{localStorage.setItem("lbs_inventory_products_cache_v8",JSON.stringify(products))}catch{}}
function loadCachedProducts(){try{const c=JSON.parse(localStorage.getItem("lbs_inventory_products_cache_v8")||"[]");if(c.length)products=c}catch{}}
function updateConnectionUI(){const el=$("connectionStatus");if(!el)return;const n=navigator.onLine;el.textContent=n?"● Online":"● Offline";el.className="connection-status "+(n?"online":"offline");const q=getQueue();el.title=q.length?`${q.length} transaksi menunggu sinkronisasi`:n?"Terhubung ke internet":"Tidak ada internet"}
async function syncPending(){if(!navigator.onLine||!currentUser)return;const q=getQueue();if(!q.length){updateConnectionUI();return}let remain=[];for(const item of q){try{if(item.type==="production"){const d=item.data;await runTransaction(db,async tx=>{const ref=doc(db,"products",d.productId),snap=await tx.get(ref);if(!snap.exists())throw Error("Produk tidak ditemukan");const cur=snap.data(),before=Number(cur.stok||0),after=before+Number(d.qty);const tr=doc(collection(db,"stock_transactions"));tx.update(ref,{stok:after,updatedAt:serverTimestamp()});tx.set(tr,{productId:d.productId,barcode:cur.barcode||d.barcode||"",kodeProduk:cur.kodeProduk||d.kodeProduk||"",namaProduk:cur.namaProduk||d.namaProduk||"",type:"production",qty:Number(d.qty),stokSebelum:before,stokSesudah:after,tanggalProduksi:d.tanggal,shift:d.shift,keterangan:d.keterangan,userId:currentUser.uid,userName:currentProfile.nama||currentUser.email,createdAt:serverTimestamp(),offlineCreatedAt:d.offlineCreatedAt||new Date().toISOString()})})}}catch(e){remain.push(item)}}saveQueue(remain);updateConnectionUI();if(q.length!==remain.length){toast(`${q.length-remain.length} transaksi offline berhasil disinkronkan`);await loadProducts();cacheProducts();if(document.querySelector('.nav.active')?.dataset.page==='production')renderProduction()}}
window.addEventListener("online",()=>{updateConnectionUI();syncPending()});window.addEventListener("offline",updateConnectionUI);
...
async function loadProducts(){try{products=(await getDocs(collection(db,"products"))).docs.map(x=>({id:x.id,...x.data()}));cacheProducts()}catch(e){loadCachedProducts();if(!products.length)throw e}}
...
function nav(page){document.querySelectorAll(".nav").forEach(x=>x.classList.toggle("active",x.dataset.page===page));$("pageTitle").textContent={dashboard:"Dashboard",products:"Master Produk",inventory:"Inventory",transactions:"Stok Masuk/Keluar",production:"Produksi",opname:"Stok Opname",reports:"Laporan",users:"Pengguna"}[page]||page;$("content").innerHTML="";({dashboard:renderDashboard,products:renderProducts,inventory:renderInventory,transactions:renderTransactions,production:renderProduction,opname:renderOpname,reports:renderReports,users:renderUsers}[page]||renderDashboard)();$("sidebar")?.classList.remove("open")}
...
async function renderProduction(){
 ...
 $("prodForm").onsubmit=async e=>{e.preventDefault();if(!selected)return alert("Scan barcode atau pilih produk terlebih dahulu.");let f=new FormData(e.target),qty=Number(f.get("qty"));if(qty<=0)return alert("Jumlah produksi harus lebih dari 0");let tanggal=f.get("tanggal"),shift=f.get("shift"),ket=f.get("keterangan").trim();try{if(!navigator.onLine){queueProduction({productId:selected.id,barcode:selected.barcode||"",kodeProduk:selected.kodeProduk||"",namaProduk:selected.namaProduk||"",qty,tanggal,shift,keterangan:ket,offlineCreatedAt:new Date().toISOString()});updateLocalProduct(selected.id,qty);toast("Offline: produksi disimpan di HP dan akan disinkronkan saat internet kembali");selected=null;await renderProduction();return}await runTransaction(db,async tx=>{let ref=doc(db,"products",selected.id),snap=await tx.get(ref);if(!snap.exists())throw Error("Produk tidak ditemukan");let d=snap.data(),before=Number(d.stok||0),after=before+qty,tr=doc(collection(db,"stock_transactions"));tx.update(ref,{stok:after,updatedAt:serverTimestamp()});tx.set(tr,{productId:selected.id,barcode:d.barcode||"",kodeProduk:d.kodeProduk||"",namaProduk:d.namaProduk||"",type:"production",qty,stokSebelum:before,stokSesudah:after,tanggalProduksi:tanggal,shift,keterangan:ket,userId:currentUser.uid,userName:currentProfile.nama||currentUser.email,createdAt:serverTimestamp()})});toast("Stok produksi berhasil ditambahkan");selected=null;await renderProduction()}catch(err){if(!navigator.onLine){queueProduction({productId:selected.id,barcode:selected.barcode||"",kodeProduk:selected.kodeProduk||"",namaProduk:selected.namaProduk||"",qty,tanggal,shift,keterangan:ket,offlineCreatedAt:new Date().toISOString()});updateLocalProduct(selected.id,qty);toast("Offline: disimpan sementara");selected=null;await renderProduction()}else alert("Gagal: "+err.message)}};
}
...
updateConnectionUI();setInterval(()=>{updateConnectionUI();if(navigator.onLine)syncPending()},15000);
onAuthStateChanged(auth,async user=>{if(!user){$("loginPage").classList.remove("hidden");$("appPage").classList.add("hidden");return}try{let s=await getDoc(doc(db,"users",user.uid));if(!s.exists()||s.data().aktif!==true){await signOut(auth);$("loginError").textContent="Akun belum terdaftar/aktif.";return}currentUser=user;currentProfile=s.data();loadCachedProducts();syncPending();$("loginPage").classList.add("hidden");$("appPage").classList.remove("hidden");$("userName").textContent=(currentProfile.nama||user.email)+" ("+(currentProfile.role||"")+")";document.querySelectorAll(".admin-only").forEach(x=>x.style.display=currentProfile.role==="admin"?"":"none");nav("dashboard")}catch(e){await signOut(auth);$("loginError").textContent="Profil pengguna tidak dapat dibaca."}});
