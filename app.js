...
window.stockForm=async(id,type)=>{
 ...
 await loadTransactions();
 const batches={};
 transactions.filter(t=>t.productId===id&&t.kodeProduksi).forEach(t=>{
   const code=String(t.kodeProduksi).trim(); if(!code)return;
   const q=Number(t.qty||0);
   if(t.type==='production'||t.type==='in') batches[code]=(batches[code]||0)+q;
   else if(t.type==='out') batches[code]=(batches[code]||0)-q;
 });
 const available=Object.entries(batches).filter(([,v])=>v>0.000001).sort((a,b)=>a[0].localeCompare(b[0]));
 const batchField=type==='out'
  ? `<div class="form-group"><label>Kode Produksi <span class="small">(opsional)</span></label><select name="kodeProduksi"><option value="">Tanpa Kode Produksi</option>${available.map(([code,bal])=>`<option value="${esc(code)}">${esc(code)} — ${money(bal)} ${esc(p.satuan||'')}</option>`).join('')}</select></div>`
  : `<div class="form-group"><label>Kode Produksi <span class="small">(opsional)</span></label><input name="kodeProduksi" placeholder="Contoh: 960726A"></div>`;
 const reasonField=type==='out'?`<div class="form-group"><label>Keperluan</label><select name="jenisKeluar"><option value="Sample">Sample</option><option value="Karantina">Karantina</option><option value="QC / Testing">QC / Testing</option><option value="Rusak">Rusak</option><option value="Internal">Internal</option><option value="Lainnya">Lainnya</option><option value="Transfer Produk">Transfer Produk</option></select></div>`:'';
 openModal(`<h2>Stok ${type==='in'?'Masuk':'Keluar'}</h2><div class="notice"><b>${esc(p.namaProduk)}</b><br>Stok saat ini: ${money(p.stok)} ${esc(p.satuan||'')}</div><form id="stockForm"><div class="form-grid">${batchField}${reasonField}${field('qty','Jumlah',1,'required','number','0.001')}${field('keterangan','Keterangan','','')}</div></form><div class="actions"><button type="button" class="btn primary" id="saveStock">Simpan</button></div>`);
 $('saveStock').onclick=async()=>{
   const form=$('stockForm'); const qty=Number(form.querySelector('[name=qty]').value); const ket=form.querySelector('[name=keterangan]').value.trim(); const kode=(form.querySelector('[name=kodeProduksi]')?.value||'').trim(); const jenisKeluar=(form.querySelector('[name=jenisKeluar]')?.value||'').trim();
   if(qty<=0)return alert('Jumlah harus lebih dari 0.');
   if(type==='out'&&qty>Number(p.stok||0))return alert(`Stok tidak mencukupi. Stok saat ini ${money(p.stok)} ${p.satuan||''}.`);
   try{
     await runTransaction(db,async tx=>{
       const ref=doc(db,'products',id); const snap=await tx.get(ref); if(!snap.exists())throw Error('Produk tidak ditemukan.');
       const d=snap.data(); const before=Number(d.stok||0); const after=type==='in'?before+qty:before-qty; if(after<0)throw Error('Stok tidak mencukupi.');
       const tr=doc(collection(db,'stock_transactions'));
       tx.update(ref,{stok:after});
       tx.set(tr,{productId:id,barcode:d.barcode||'',kodeProduk:d.kodeProduk||'',namaProduk:d.namaProduk||'',kodeProduksi:kode||null,type,qty,stokSebelum:before,stokSesudah:after,keterangan:ket,jenisKeluar:jenisKeluar||null,userId:currentUser.uid,userName:currentProfile.nama||currentUser.email,createdAt:serverTimestamp()});
     });
     closeModal(); toast(type==='out'?'Stok berhasil dikeluarkan':'Stok berhasil ditambahkan'); await loadProducts(); await loadTransactions(); renderInventory();
...
window.transferStockForm=async sourceId=>{
 ...
 $('saveTransfer').onclick=async()=>{
   ...
   try{await runTransaction(db,async tx=>{
     ...
     const sa=sb-qty, da=dbefore+qty, transferId='TRF-'+Date.now()+'-'+Math.random().toString(36).slice(2,8);
...
function openTransactionPicker(title,callback){openModal(`<h2>${esc(title)}</h2><div class="form-group"><label>Produk</label><select id="trxProduct" required><option value="">Pilih produk</option>${products.filter(p=>p.aktif!==false).sort((a,b)=>String(a.namaProduk||'').localeCompare(String(b.namaProduk||''),'id')).map(p=>`<option value="${esc(p.id)}">${esc(p.namaProduk)} — ${money(p.stok)} ${esc(p.satuan||'')}</option>`).join('')}</select></div><div class="actions"><button type="button" class="btn primary" id="trxContinue">Lanjut</button></div>`);$("trxContinue").onclick=()=>{const id=$("trxProduct").value;if(!id)return alert("Pilih produk terlebih dahulu.");closeModal();callback(products.find(p=>p.id===id))}}
