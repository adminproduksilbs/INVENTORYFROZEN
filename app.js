...
window.stockForm=async(id,type)=>{
 ...
 openModal(`<h2>Stok ${type==="in"?"Masuk":"Keluar"}</h2><div class="notice"><b>${esc(p.namaProduk)}</b><br>Stok saat ini: ${money(p.stok)} ${esc(p.satuan)}${available.length?`<br><small>Batch tersedia: ${available.length} kode produksi</small>`:""}</div><form id="stockForm"><div class="form-grid">${batchField}${field("qty","Jumlah",1,"required","number","0.001")}${field("keterangan","Keterangan","","")}</div></form><div class="actions"><button class="btn primary" id="saveStock">Simpan</button></div>`);
 $("saveStock").onclick=async()=>{const form=$("stockForm"),qty=Number(form.querySelector('[name=qty]').value),ket=form.querySelector('[name=keterangan]').value.trim(),kode=(form.querySelector('[name=kodeProduksi]')?.value||'').trim();if(qty<=0)return alert("Jumlah harus lebih dari 0");if(type==="in"&&!kode)return alert("Kode Produksi wajib diisi untuk Stok Masuk.");if(type==="out"&&kode){const bal=batches[kode]||0;if(qty>bal+1e-9)return alert(`Stok batch ${kode} hanya tersisa ${money(bal)} ${p.satuan}.`)}try{await runTransaction(db,async tx=>{let ref=doc(db,"products",id),snap=await tx.get(ref);if(!snap.exists())throw Error("Produk tidak ditemukan");let d=snap.data(),before=Number(d.stok||0),after=type==="in"?before+qty:before-qty;if(after<0)throw Error("Stok tidak mencukupi");let tr=doc(collection(db,"stock_transactions"));tx.update(ref,{stok:after});tx.set(tr,{productId:id,barcode:d.barcode||"",kodeProduk:d.kodeProduk||"",namaProduk:d.namaProduk||"",kodeProduksi:kode,type,qty,stokSebelum:before,stokSesudah:after,keterangan:ket,userId:currentUser.uid,userName:currentProfile.nama||currentUser.email,createdAt:serverTimestamp()})});closeModal();toast("Transaksi berhasil");renderInventory()}catch(e){alert("Gagal: "+e.message)}}
}
