...
async function renderDashboard(){
  ...
  $('content').innerHTML=`
    ...
    <div class="stats dashboard-kpis">
      <div class="card kpi kpi-blue"><div class="kpi-icon">▣</div><div><span>Total Stok</span><strong>${money(stok)}</strong><small>total semua produk</small></div><div class="kpi-trend">↗ Data terkini</div></div>
      ...
      <div class="card kpi kpi-purple"><div class="kpi-icon">◉</div><div><span>Total Produk</span><strong>${money(total)}</strong><small>produk terdaftar</small></div><div class="kpi-trend">↗ Data terkini</div></div>
    ...
    <div class="dashboard-grid">
      ...
      <div class="card chart-card">
        <div class="section-title"><div><h3>◔ Komposisi Produk</h3><p>Persentase jumlah stok berdasarkan jenis produk.</p></div></div>
...
function drawDashboardCharts(labels,masuk,keluar,catRows){
  ...
  const cc=$('categoryChart'); if(cc){new Chart(cc,{type:'doughnut',data:{labels:catRows.map(x=>x[0]),datasets:[{data:catRows.map(x=>x[1]),borderWidth:3,borderColor:'#fff'}]},options:{responsive:true,maintainAspectRatio:false,cutout:'64%',plugins:{legend:{display:false},tooltip:{callbacks:{label:c=>`${c.label}: ${money(c.raw)} stok`}}}}});}
}
